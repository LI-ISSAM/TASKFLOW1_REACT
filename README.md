...
**Règle :** même entrée → même sortie, sans toucher à rien d'extérieur.

```ts
// ❌ MAUVAIS — effet de bord dans le reducer
case 'LOGIN_SUCCESS':
  localStorage.setItem('user', JSON.stringify(action.payload)); // INTERDIT
  return { user: action.payload, loading: false, error: null };

// ✅ BON — calcul pur uniquement
case 'LOGIN_SUCCESS':
  return { user: action.payload, loading: false, error: null };
```
...
```
// Sans check → crash mystérieux plus tard
Cannot read properties of undefined (reading 'user')

// Avec check → message clair immédiat
...
async function handleSubmit(e: React.FormEvent) {
  e.preventDefault(); // ← bloque le rechargement natif
  // Notre logique async prend le relais
...
// user contient :
{ id: "1", email: "admin@taskflow.com", name: "Admin" }  // ← sans le password
```
...
```tsx
// ❌ Si Dashboard était dans App :
export default function App() {
  const [projects, setProjects] = useState([]); // s'exécute TOUJOURS
  useEffect(() => {
    fetch('http://localhost:4000/projects') // fetch AVANT connexion → erreur
  }, []);
...
// ✅ Avec Dashboard séparé :
export default function App() {
  if (!authState.user) return <Login />;
  return <Dashboard />; // Dashboard monté → ses hooks s'exécutent
}
```
